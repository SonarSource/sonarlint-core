...
* Add opt-in local discovery of supported AI agent CLIs to `getIntegrationState`, including IDE/CLI detection sources and dedicated GitHub Copilot CLI and Antigravity targets. `AiIntegrationAgentCapability` now requires `detectionSources`; clients must pass the detected sources instead of relying on an implicit IDE default.
