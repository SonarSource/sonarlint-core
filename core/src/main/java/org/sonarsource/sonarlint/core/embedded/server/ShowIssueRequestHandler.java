...
public class ShowIssueRequestHandler extends ShowHotspotOrIssueRequestHandler implements HttpRequestHandler {
  ...
  private void showIssue(ShowIssueQuery query) {
    ...
    if (connectionsMatchingOrigin.isEmpty()) {
      ...
      try {
        var assistNewConnectionResult = assistCreatingConnection(query.serverUrl, query.tokenName, query.tokenValue).join();
        var assistNewBindingResult = assistBinding(assistNewConnectionResult.getNewConnectionId(), query.projectKey).join();
