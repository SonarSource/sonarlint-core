...
public final class StandaloneSonarLintEngine extends AbstractSonarLintEngine {

  private static final Logger LOG = Loggers.get(StandaloneSonarLintEngine.class);
  ...
  public StandaloneSonarLintEngine(StandaloneGlobalConfiguration globalConfig) {
  ...
  private void loadPluginMetadata() {
    ...
    allRulesDefinitionsByKey = ruleExtractor.extractRules(globalConfig.getPluginJarPaths(), globalConfig.getEnabledLanguages()).stream()
      .collect(toMap(SonarLintRuleDefinition::getKey, r -> r));
  }
  ...
  public AnalysisResults analyze(StandaloneAnalysisConfiguration configuration, Consumer<Issue> issueListener, @Nullable LogOutput logOutput, @Nullable ProgressMonitor monitor) {
    ...
    Collection<ActiveRule> activeRules = filteredActiveRules.stream().map(rd -> {
      ...
      rd.getParams().forEach((paramKey, paramDef) -> effectiveParams.put(paramKey, paramDef.defaultValue()));
      Optional.ofNullable(configuration.ruleParameters().get(rd.getKey())).ifPresent(effectiveParams::putAll);
