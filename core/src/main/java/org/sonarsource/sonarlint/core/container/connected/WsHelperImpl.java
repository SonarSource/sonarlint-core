...
public class WsHelperImpl implements WsHelper {
  private static final String TOKEN_NAME = "SonarLint_IntelliJ_automatically_generated";
  ...
  @Override
  public String generateAuthenticationToken(ServerConfiguration serverConfig) {
    checkNotNull(serverConfig);
    try {
      SonarLintWsClient client = new SonarLintWsClient(serverConfig);

      // revoke, ignore result
      client.post("api/user_tokens/revoke?name=" + TOKEN_NAME);

      // create
      CloseableWsResponse response = client.post("api/user_tokens/generate?name=" + TOKEN_NAME);
