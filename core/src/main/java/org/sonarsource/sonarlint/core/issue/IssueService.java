...
public class IssueService {
  ...
  public ReopenIssueResponse reopenIssue(ReopenIssueParams params, CancelChecker cancelChecker) {
    ...
    if (isServerIssue) {
      return reopenServerIssue(serverApiConnection, issueId, projectServerIssueStore, cancelChecker);
    } else {
  ...
  private ReopenIssueResponse reopenServerIssue(ServerApi connection, String issueId, ProjectServerIssueStore projectServerIssueStore, CancelChecker cancelChecker) {
    waitForHttpRequest(cancelChecker, connection.issue().changeStatusAsync(issueId, Transition.REOPEN), "Reopen server issue");
    var serverIssue = projectServerIssueStore.updateIssueResolutionStatus(issueId, false, false);
