...
class UserSynchronizationMediumTests {
  ...
  void it_should_store_user_id_on_sonarcloud(SonarLintTestHarness harness) {
    var backend = harness.newBackend()
      ...
      .withSonarQubeCloudEuRegionUri("http://localhost:12345")
      .withSonarQubeCloudEuRegionApiUri("http://localhost:12345")
      .withSonarCloudConnection("connectionId", "myOrg")
      .withBackendCapability(FULL_SYNCHRONIZATION)
      .start();

    // the fake server in medium-tests does not currently expose users/current; this test assumes
    // the synchronizer is tolerant to missing endpoint and simply not create the file
