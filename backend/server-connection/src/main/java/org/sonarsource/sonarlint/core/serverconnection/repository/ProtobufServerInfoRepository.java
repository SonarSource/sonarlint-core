...
/**
 * Protobuf-based implementation of ServerInfoRepository.
 */
public class ProtobufServerInfoRepository implements ServerInfoRepository {
  public static final String SERVER_INFO_PB = "server_info.pb";
  private static final SonarLintLogger LOG = SonarLintLogger.get();

  private final Path storageRoot;

  public ProtobufServerInfoRepository(Path storageRoot) {
    this.storageRoot = storageRoot;
  }

  private Path getStorageFilePath(String connectionId) {
    return storageRoot.resolve(encodeForFs(connectionId)).resolve(SERVER_INFO_PB);
  }

  @Override
  public void store(String connectionId, ServerStatusInfo serverStatus, Set<Feature> features, Map<String, String> globalSettings) {
  ...
  public Optional<StoredServerInfo> read(String connectionId) {
    var storageFilePath = getStorageFilePath(connectionId);
    return new RWLock().read(() -> Files.exists(storageFilePath) ? Optional.of(adapt(ProtobufFileUtil.readFile(storageFilePath, Sonarlint.ServerInfo.parser())))
      : Optional.empty());
  }
