...
public class FindingReportingService {
  ...
  public FindingReportingService(SonarLintRpcClient client, ConfigurationRepository configurationRepository, NewCodeService newCodeService,
    StorageService storageService, PreviouslyRaisedFindingsRepository previouslyRaisedFindingsRepository) {
    this.client = client;
  ...
  private boolean isMQRModeForConfigurationScope(String configurationScopeId) {
    var effectiveBindingOpt = configurationRepository.getEffectiveBinding(configurationScopeId);
    return effectiveBindingOpt.flatMap(binding -> storageService.connection(binding.getConnectionId()).serverInfo().read().map(StoredServerInfo::isMQRMode)).orElse(false);
  }
}
