...
public class AnalysisExecutor {
  ...
  void execute(AnalysisTask task) {
    try {
      ...
      task.getResult().completeExceptionally(e);
    }
  }
  ...
  void doExecute(AnalysisTask task) {
    ...
    engine.get().post(analyzeCommand, task.getProgressMonitor())
      .handle((res, err) -> {
        var endTime = System.currentTimeMillis();
        if (err != null) {
          LOG.error("Error during analysis", err);
          ...
          eventPublisher.publishEvent(new AnalysisFinishedEvent(task.getAnalysisId(), configScopeId, analysisDuration,
            languagePerFile, res.failedAnalysisFiles().isEmpty(), rawIssues, task.isShouldFetchServerIssues()));
          task.getResult().complete(new AnalysisResult(
            res.failedAnalysisFiles().stream().map(ClientInputFile::getClientObject).map(clientObj -> ((ClientFile) clientObj).getUri()).collect(Collectors.toSet()), rawIssues));
        }
        return res;
      });
  }
