...
final class AgentCliLocator {
  ...
  List<AiAgent> discover(@Nullable String resolvedPath) {
    var discovered = new ArrayList<AiAgent>();
    var directories = executableSearchDirectories(resolvedPath);
    for (var profile : AgentProfiles.cliDiscoverable()) {
      if (isAgentCliInstalled(profile, directories, resolvedPath)) {
        discovered.add(profile.agent());
      }
    }
    return discovered;
  }
  ...
  private boolean isAgentCliInstalled(AgentProfile profile, Set<Path> directories, @Nullable String resolvedPath) {
    for (var baseName : profile.executableNames()) {
      var candidate = firstExistingExecutable(baseName, directories);
      if (candidate != null && probeMatches(candidate, profile, resolvedPath)) {
        return true;
      }
    }
    return false;
