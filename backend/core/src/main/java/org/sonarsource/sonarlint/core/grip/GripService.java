...
public class GripService {
  ...
  public SuggestFixResponse suggestFix(SuggestFixParams params) {
    ...
    return doSuggestFix(params, fileContent);
  }

  private SuggestFixResponse doSuggestFix(SuggestFixParams params, String fileContent) {
  ...
  public SuggestFixesResponse suggestFixes(SuggestFixesParams params) {
    var fileUri = params.getFileUri();
    var clientFile = fileSystemService.getClientFiles(params.getConfigurationScopeId(), fileUri);
    if (clientFile == null) {
      throw new IllegalStateException("Cannot find the file with URI: " + fileUri);
    ...
    var fileContent = clientFile.getContent();
    return new SuggestFixesResponse(params
      .getIssuesToFix().parallelStream().map(issueToFix -> new SuggestFixesResponse.Result(doSuggestFix(new SuggestFixParams(params.getServiceURI(), params.getAuthenticationToken(), params.getPromptId(),
        params.getConfigurationScopeId(), params.getFileUri(), issueToFix.getMessage(), issueToFix.getRange(), issueToFix.getRuleKey()), fileContent).getResult()))
