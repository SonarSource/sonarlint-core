...
public class McpConfigurationService {
  ...
  public McpConfigurationUpdatePlanResponse planUpdate(McpConfigurationUpdateParams params) {
    ...
    if (content == null || content.isBlank()) {
      return plan(McpConfigurationState.NOT_CONFIGURED,
        canonicalDocument(sectionName, desiredEntry), List.of());
    }
    var parsed = parse(content, sectionName);
    if (parsed.error != null) {
    ...
    return plan(state,
      updatedDocument(updateRoot(parsed.root, sectionName, desiredEntry)), List.of());
  }
  ...
  private static String canonicalDocument(String sectionName, JsonNode entry) {
  ...
  private static ObjectNode sectionWithEntry(JsonNode entry) {
