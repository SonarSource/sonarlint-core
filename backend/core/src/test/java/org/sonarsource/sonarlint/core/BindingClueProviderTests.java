...
class BindingClueProviderTests {
  ...
  void should_detect_sonar_scanner_for_sonarcloud_based_on_url() {
    ...
    when(connectionRepository.getConnectionById(SC_CONNECTION_ID_1)).thenReturn(new SonarCloudConnectionConfiguration(SonarCloudActiveEnvironment.PRODUCTION_URI, SC_CONNECTION_ID_1, MY_ORG_1, true));
    when(connectionRepository.getConnectionById(SC_CONNECTION_ID_2)).thenReturn(new SonarCloudConnectionConfiguration(SonarCloudActiveEnvironment.PRODUCTION_URI, SC_CONNECTION_ID_2, MY_ORG_2, true));
  ...
  void should_not_detect_sonarlint_configuration_file_if_not_json() {
    var file = new ClientFile(URI.create("/path/to/.sonarlint/connectedMode.txt"), CONFIG_SCOPE_ID, Path.of("/path/to/.sonarlint/connectedMode.txt"), false, null, null);
  ...
  void should_not_detect_sonarlint_configuration_file_if_wrong_folder() {
    var file = new ClientFile(URI.create("/path/to/.sonarlint/connectedMode.json"), CONFIG_SCOPE_ID, Path.of("/path/to/.sonarlint2/connectedMode.json"), false, null, null);
