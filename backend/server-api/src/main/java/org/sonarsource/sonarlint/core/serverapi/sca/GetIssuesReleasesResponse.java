...
public record GetIssuesReleasesResponse(List<IssuesRelease> issuesReleases, Page page) {
  public record IssuesRelease(UUID key, Type type, Severity severity, Release release, List<Transition> transitions) {
    public record Release(String packageName, String packageVersion) {
    }
