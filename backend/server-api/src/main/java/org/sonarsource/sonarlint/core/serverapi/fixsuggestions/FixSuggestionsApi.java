...
public class FixSuggestionsApi {
  ...
  public AiSuggestionResponseBodyDto getAiSuggestion(AiSuggestionRequestBodyDto dto, SonarLintCancelMonitor cancelMonitor) {
    ...
    var stringBody = gson.toJson(dto);
    stringBody = stringBody.replace("\\n", "\n");
    try (var response = helper.apiPost("/fix-suggestions/ai-suggestions", JSON_CONTENT_TYPE, stringBody, cancelMonitor)) {
  ...
  public SupportedRulesResponseDto getSupportedRules(SonarLintCancelMonitor cancelMonitor) {
    try (var response = helper.get("/api/fix-suggestions/supported-rules", cancelMonitor)) {
