...
class NewCodeApiTests {
  ...
  void parseReferenceBranchPeriod() {
    ...
    var newCodeDefinition = underTest.getNewCodeDefinition(PROJECT, BRANCH, new SonarLintCancelMonitor()).orElseThrow();
  ...
  void parseUnknownModePeriod() {
    prepareSqWsResponseWithPeriod(Measures.Period.newBuilder()
      ...
      .build());
    assertThat(underTest.getNewCodeDefinition(PROJECT, BRANCH, new SonarLintCancelMonitor())).isEmpty();
  }
