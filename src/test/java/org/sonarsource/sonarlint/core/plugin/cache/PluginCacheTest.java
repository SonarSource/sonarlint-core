...
public class PluginCacheTest {
  ...
  public void not_in_cache() throws IOException {
    PluginCache cache = PluginCache.create(tempFolder.newFolder());
  ...
  public void download_and_add_to_cache() throws IOException {
    ...
    PluginCache cache = new PluginCache(tempFolder.newFolder(), hashes);
  ...
  public void concurrent_download() throws IOException {
    PluginHashes hashes = mock(PluginHashes.class);
    when(hashes.of(any(File.class))).thenReturn("ABCDE");
    final PluginCache cache = new PluginCache(tempFolder.newFolder(), hashes);
